...
export default function Search() {
    ...
    return(
        <div className="bg-gray-800 border-b-2 border-gray-700 rounded-tl-lg">
            <div className="p-2 flex gap-8 h-16 items-center">
                <h2 className="text-gray-300">SAFARI Chat</h2>
                <div className="gap-2 flex">
                    <img className="rounded-lg w-6" src={currentUser.photoURL} alt=""/>
                    <h2 className="text-gray-300">{currentUser.displayName}</h2>
                </div>
            </div>
            <div>
                <input type="text" placeholder="find a user" onKeyDown={handleKey}
                onChange={(e) => setUsername(e.target.value)} value={username} className="bg-slate-800 text-white p-2 text-sm w-full h-8"/>
            </div>
