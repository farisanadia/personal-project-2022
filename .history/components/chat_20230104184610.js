...
export default function Chat({children, avatar, username, text, timestamp, user}) {
    ...
    return ( checkSender() ?
        <div className="bg-cyan-200 p-4 rounded-lg text-black w-3/12 border-white border-4">
            <p className="py-1 space-y-2 text-s break-all">{text}</p>
            {children}
        </div>
    : 
    <div className="p-4 p-4 rounded-lg text-black w-3/12 border-white border-4">
