...
export default function Chat({children, avatar, username, text, timestamp, user}) {
    ...
    return ( checkSender() ?
        <div className="p-4 rounded-lg bg-cyan-200 text-black flex flex-row-reverse w-3/12 border-white border-4">
            <p className="float-right items-center py-1 space-y-2 text-s break-all">{text}</p>
            {children}
        </div>
    : 
