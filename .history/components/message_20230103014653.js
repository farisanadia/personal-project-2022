...
export default function Message({children, avatar, username, description, timestamp, title}) {
    return(
        <div className="bg-white p-4 border-2 rounded-lg">
            ...
            <div className="items-center py-3 space-y-2">
                <p className="text-md break-all">{title}</p>
                <article class="prose lg:prose-s break-all">
                    {description}
                </article>
                <p className="text-sm text-gray-900 break-all">{description}</p>
            </div>
            {children}
        </div>
    );
