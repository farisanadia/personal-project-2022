...
export default function Details() {
    ...
    const checkedLikedBy = async() => {
        for (var i = 0; i < likedBy?.length; i++) { 
            if (likedBy[i].userName == auth.currentUser.displayName) { return true };
        }
    ...
    return (
        <div>
            ...
            className={`"items-center justify-center gap-2 py-2 text-small flex
            ${likedBy.length > 0 && likedBy.includes() ? 'text-green-600' : 'text-black-600'}`}>
