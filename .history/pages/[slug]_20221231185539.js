...
export default function Details() {
    ...
    const getLikes = async() => {
        const docRef = doc(db, 'posts', routeData.id);
        const unsubscribe = onSnapshot(docRef, (snapshot) => {
            if (Number.isNaN(likes)) setLikes(0);
            else setLikes(snapshot.data.likes);
        })
        return unsubscribe;
    }
    ...
    return (
        <div>
            <Message {...routeData} {...likes}></Message> 
            <div>
                <button onClick={submitLike}>{setLikes.FieldValue < 0 ? 0 : likes} Likes</button>
            </div>
            <div className="my-4">
                <div className="flex">
