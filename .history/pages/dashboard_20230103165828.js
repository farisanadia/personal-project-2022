            ...
            <div className="space-y-2">
                {posts.map(post => {
                    ...
                    <Message {...post} key={post.id}>
                        ...
                        >
                        <Menu.Items className="absolute flex right-0 z-10 mt-2 w-40 origin-top-right rounded-md p-2 bg-white shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none">
                            <div className="py-1">
                                <Menu.Item>
                                    {({ active }) => (
                                        <button onClick={() => deletePost(post.id)} 
                                        className={`'items-center gap-2 block px-4 py-2 text-sm ${active ? 'bg-gray-100 text-gray-900' : 'text-gray-700'}`}>
                                            <BsTrash2Fill className="inline-flex items-center text-l"/>
                                            Delete
                                        </button>
                                    )}
                                </Menu.Item>
                                ...
                                {({ active }) => (
                                    <Link href={{pathname: '/post', query:post }}>
                                        <button className={`'items-center block px-4 py-2 text-sm ${active ? 'bg-gray-100 text-gray-900' : 'text-gray-700'}`}>
                                            <AiFillEdit className="inline-flex items-center text-l"/>
                                            Edit
                                        </button>
                                    </Link>
                                )}
